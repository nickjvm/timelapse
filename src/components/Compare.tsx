            ...
            <div className="flex items-center justify-center">
                {mode === 'overlaid' && (
                    <div ref={containerRef} className="relative items-center justify-center aspect-[calc(3/4)]">
                        <div>
                            <FrameImage projectId={projectId} id={leftFrameId} ratio="aspect-[calc(3/4)]" className="w-md" alt="" />
                            <p className="absolute bottom-0 left-0 right-0 text-right text-lg text-shadow-lg font-bold text-white p-2">{leftFrame?.caption}</p>
                        </div>
                        <div className="absolute top-0 left-0 overflow-hidden" style={{ width: compareWidth + 'px' }}>
                            <FrameImage projectId={projectId} id={rightFrameId} ratio="aspect-[calc(3/4)]" className="w-md" alt="" />
                            <p className="absolute bottom-0 left-0 right-0 text-left text-lg text-shadow-lg font-bold text-white p-2 whitespace-nowrap overflow-hidden">{rightFrame?.caption}</p>
                        </div>
                        <motion.div
                            drag
                            style={{ x }}
                ...
                {mode === 'side-by-side' && (
                    <div className="relative items-center justify-center flex gap-2">
                        <div className="relative">
                            <FrameImage projectId={projectId} id={leftFrameId} ratio="aspect-[calc(3/4)]" className="w-md" alt="" />
                            <p className="absolute top-full left-0 right-0 text-center text-xl font-bold text-white p-2">{leftFrame?.caption}</p>
                        </div>
                        <div className="relative">
                            <FrameImage projectId={projectId} id={rightFrameId} ratio="aspect-[calc(3/4)]" className="w-md" alt="" />
                            <p className="absolute top-full left-0 right-0 text-center text-xl font-bold text-white p-2">{rightFrame?.caption}</p>
                        </div>
                    </div>
                )}
            </div>
